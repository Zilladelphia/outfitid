    ...
    <div className="md:hidden">
      ...
      {open && (
        <div className="absolute top-16 left-0 right-0 bg-black border-b border-zinc-800 px-4 py-4 flex flex-col gap-4 z-50">
          ...
          <Link href="/" onClick={() => setOpen(false)} className="text-zinc-400 hover:text-white transition">Home</Link>
          <Link href="/shows" onClick={() => setOpen(false)} className="text-zinc-400 hover:text-white transition">Shows</Link>
          <Link href="/actors" onClick={() => setOpen(false)} className="text-zinc-400 hover:text-white transition">Actors</Link>
        </div>
      )}
    </div>
  )
